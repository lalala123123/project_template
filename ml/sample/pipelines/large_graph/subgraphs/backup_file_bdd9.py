...
print(datetime.now(), "Declaring pipeline: backup_file_bdd9")
@dsl.pipeline(name='backup_file_bdd9', default_compute_target='cpu-cluster')
def backup_file_bdd9(
    sourcepath,
...
if __name__ == '__main__':
    print(datetime.now(), "Creating backup_file_bdd9")
    pipeline = backup_file_bdd9()
    print(datetime.now(), "Validating")
    pipeline.validate()
    print(datetime.now(), "Saving")
    pipeline._save()
    print(datetime.now(), "Submitting")
    pipeline.submit(experiment_name='backup_file_bdd9')
